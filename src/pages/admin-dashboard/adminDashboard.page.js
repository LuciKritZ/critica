...
const AdminDashboard = () => {
    ...
    const sendRequest = (isAllowed, key) => {
        axios
            .post(`${process.env.REACT_API_URL}adminresponse`, {
                respond: isAllowed,
                userID: key,
            })
            .then(() => {
                const constAdminData = adminData;
                const dataIndex = constAdminData.findIndex(
                    (eachAdminData) => eachAdminData.id === key,
                );
                if (dataIndex !== -1) {
    ...
    useEffect(() => {
        if (authenticated && userId && role === 3) {
            setisLoading(true);
            axios
                .get(`${process.env.REACT_API_URL}criticsrequests?limit=100&offset=0`)
                .then((response) => {
                    setisLoading(false);
